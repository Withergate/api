...
INSERT INTO placeholder_texts(code, lang, text) VALUES
    ...
    ('encounter.w.10.fail', 'en', 'After a while, he wasn’t alive at all. The character found out there is no water so it is neccessary to return back. The character was terribly thirsty, but - unlike the ex-wanderer - stayed alive. '),
    ...
    ('encounter.c.6.succ', 'en', 'The only way how to deal with them is to cut off a hand of one of them. And it works, the second one is running away! After this situation the character fitted into local society quite easily, and found out interesting information. Even though it seems that it will be better not going there ever again. '),
    ...
    ('encounter.c.6.fail', 'en', 'The couple gave the character a few slaps and threw the character out of the tavern.'),
    ...
    ('encounter.c.8.desc', 'en', 'One of the houses in the city looked quited unpillaged and quite stably...'),
    ('encounter.c.8.desc', 'cs', 'Ten dům vypadal docela nevypleněně a skoro stabilně...'),
    ('encounter.c.8.succ', 'en', 'But it can fall into pieces anytime! The character searched the house carefully and found a few valuables which weren’t discovered by the marauders yet.'),
    ...
    ('encounter.c.8.fail', 'en', 'But was not stable at all. The building collapsed when the character was inside looking for valuables. The character was lucky and escaped without injury. But outside was an officer who fined the character, because it is probably against the law when you destroy somebody’s building.'),
    ...
    ('encounter.c.9.desc', 'en', 'The locals are playing some game in the street. One man moves with three shells and the player has to guess under which shell is hidden a small cogwheel. Left, middle or right?'),
